...
void menu(int count) {
  ...
  cout << RED << BOLD << "1)" << RESET << BOLD << "Checking whether this item is included in the container" << endl;
  cout << RED << BOLD << "2)" << RESET << BOLD<< "Print all element" << endl;
  cout << RED << BOLD << "3)" << RESET << BOLD << "Default values" << endl;
  cout << RED << BOLD << "4)" << RESET << BOLD << "Exit" << endl;
  cout << YELLOW << BOLD << "Your answer: " << RESET << BOLD;
}
...
int choose(int a, int b) {
  ...
  if(!(std::cin >> choose) || choose < a || choose > b) {
    std::cout << RED << BOLD << "\nERROR with convert to int or incorrenct number" << RESET << std::endl;
    return pack::ERROR;
  }
  std::cout << RESET;
...
void menu_for_actions() {
  std::cout << GREEN << "Choose one of these options:" << std::endl;
  menu_the_pole();
}
